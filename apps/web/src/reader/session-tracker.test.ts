/**
 * session-tracker.test.ts — pure unit tests for createSessionTracker.
 *
 * All timing is fake (injected now / tzOffset); no jsdom needed.
 * Reducer math is tested here; the hook test only checks plumbing.
 */
...
describe('createSessionTracker', () => {
  ...
  it('page events accumulate distinct ascending pages and advance active time; repeat page no dup', () => {
    ...
    expect(s.activeMs).toBe(30_000); // 3 × 10s gaps (open page counts as initial, first page event at +10s)
  });
